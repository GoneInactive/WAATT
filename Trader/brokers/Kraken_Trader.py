...
class Kraken_Trader:
    def __init__(self):
        self.asset = "BTC/USD"
        self.orderbook_client = OrderbookClientV1()  # Assuming an orderbook client is needed for API calls

    async def get_book(self):
        """Gets the book of self.asset and prints it out"""
        book = await self.orderbook_client.get(pair=self.asset)  # Fetch the order book
        bid = list(book["bid"].items())  # Extract the bid side of the book
        ask = list(book["ask"].items())  # Extract the ask side of the book

        print("Bid         Volume\t\t Ask         Volume")
        for level in range(min(len(bid), len(ask))):  # Print both sides up to the smaller depth
            print(
...
async def main():
    trader = Kraken_Trader()
    await trader.get_book()  # Print the book for BTC/USD
